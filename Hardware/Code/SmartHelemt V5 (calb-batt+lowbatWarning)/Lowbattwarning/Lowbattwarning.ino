...
void showBatteryEmptyWarning() {
  ...
  // Draw large battery icon with X
  display.drawRect(40, 10, 48, 24, SSD1306_WHITE);
  display.fillRect(88, 16, 8, 12, SSD1306_WHITE);  // Battery tip
  
  // Draw X inside battery
...
void updateBatteryReading() {
  ...
  if (currentBatteryVoltage > previousBatteryVoltage + 0.1) {
    isCharging = true;
    lowBatteryWarningShown = false;    // Reset warnings when charging
  ...
  if (currentBatteryPercent <= 0 && !isCharging && !emptyBatteryWarningShown) {
    ...
    // Enter deep sleep after 10 seconds to protect battery
    delay(10000);
    enterDeepSleep();
  }
...
int voltageToPercentage(float vBattery) {
  // Use safe minimum (3.05V) as 0%, not absolute minimum (3.0V)
  // This reserves 5% as safety buffer
  int percent = (int)((vBattery - BATTERY_SAFE_MIN) / (BATTERY_FULL - BATTERY_SAFE_MIN) * 100.0);
  
  if (percent > 100) percent = 100;
  if (percent < 0) percent = 0;  // Show 0% when at 3.05V (actual 5%)
